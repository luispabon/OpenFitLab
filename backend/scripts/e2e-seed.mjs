#!/usr/bin/env node
/**
 * E2E seed script — creates a disposable test user in MariaDB and injects a
 * pre-signed session into Valkey so Playwright can access authenticated endpoints.
 *
 * Run inside the api container (has all node_modules + network access to db/valkey):
 *   docker compose exec -T api node scripts/e2e-seed.mjs
 *
 * Outputs a single line:  SESSION_COOKIE=<signed-cookie-value>
 *
 * The caller (global-setup.ts) then fetches the CSRF token from /api/auth/me and
 * uploads the seed event file via /api/events.
 */
